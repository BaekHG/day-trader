"""
Force-close failsafe — 메인 봇과 독립적으로 15:15에 실행.
positions.json에 포지션이 남아있으면 무조건 시장가 매도.
봇이 크래시/hang 되어도 오버나이트 방지.
...
def clear_positions_file():
    """청산 후 positions.json 초기화."""
    try:
        tmp = config.POSITIONS_FILE + ".tmp"
        with open(tmp, "w") as f:
            json.dump({}, f)
        os.rename(tmp, config.POSITIONS_FILE)
        logger.info("positions.json 초기화 완료")
    except Exception as e:
        logger.error("positions.json 초기화 실패: %s", e)
...
def force_close_all(dry_run: bool = False):
    ...
    logger.info("강제청산 failsafe 완료 — 성공 %d / 실패 %d", success_count, fail_count)
