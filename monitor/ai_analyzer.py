...
제공된 데이터를 기반으로 판단하되, 장전/장중 상황에 맞게 분석하라.
...
장 시작 전 분석 (08:00~09:00) — 중요:
- 당일 거래량/거래대금이 0인 것은 정상이다. 아직 장이 열리지 않았기 때문이다.
- "거래량이 없어서 분석 불가" 또는 "데이터 부족으로 매매비추천"이라고 판단하지 마라.
- 장전 분석의 본질은 전일 데이터와 뉴스 기반 예측이다.
- 핵심 판단 기준: 전일 종가 기준 일봉 추세(최근 5일), 외국인/기관 수급 연속성, 뉴스 모멘텀, 테마 흐름
- 전일 거래대금이 높았던 종목 중 수급 유입 + 뉴스 모멘텀 있는 종목 우선 선별
- 전일 캔들 패턴(양봉/음봉, 거래량 변화)으로 금일 흐름 예측
- 갭상승/갭하락 가능성 평가
- 장전이더라도 확신 있는 종목이 있으면 반드시 매매추천하라
...
- 손절 미준수 전략 금지
- 낙관적 시나리오만 제시 금지
- 장중일 때: 가격 조건 + 거래대금 조건 + 시간 조건을 반드시 포함
- 장전일 때: 가격 조건 + 전일 거래대금 + 수급/뉴스 근거를 반드시 포함
    ...
    def _build_user_prompt(
        ...
        if is_market_open:
            lines.append(
                ...
                "일봉 추세, 수급 연속성, 고점대비 위치, 5분봉 눌림목 패턴, "
                "거래대금 추세를 종합 판단하세요. "
            ...
            lines.append(
                "위 전일 데이터와 뉴스를 기반으로 오늘 장 시작 시 단타 매매에 가장 적합한 TOP 3 종목을 선정하고, "
                "각 종목별 포트폴리오 배분 비율(합계 100%)을 제시하세요. "
                "당일 거래량/거래대금이 0인 것은 장 시작 전이므로 정상이며, 이를 이유로 매매비추천하지 마세요. "
                "전일 일봉 추세(양봉/음봉, 거래량 변화), 외국인/기관 수급 연속성, "
                "뉴스 모멘텀, 고점대비 위치, 전일 거래대금 수준을 종합하여 "
                "장 시작 시 상승 가능성이 높은 종목을 선별하세요. "
                "확신이 높은 종목에 더 높은 비중을 배분하세요. "
                "반드시 데이터 기반으로 판단하고, JSON 형식으로 응답하세요."
            )
