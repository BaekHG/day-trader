...
트레일링 보호선: +0.8%→+0.4%, +1.5%→+1.0%, +2%→+1.5%, +3%→+2.2%, +5%→+4%.
...
장중 분석 (09:10 이후 — 5분봉 2개 확인 후):
- 09:00~09:05 첫 봉은 카오스 — 무시. 09:05~09:10 두 번째 봉까지 확인 후 판단.
- 5분봉 패턴 필수 확인:
  ✅ 진입 가능: 양봉 2연속 + 거래량 증가, 눌림목 후 반등(음봉→양봉 전환+거래량 동반), 직전 고점 돌파+거래량 급증
  ❌ 진입 금지: 음봉 2연속(하락 추세), 윗꼬리 긴 봉(매도 압력), 거래량 감소 추세, 고점 대비 급락 중
- 거래대금 추이: 5분봉별 거래량이 감소하면 모멘텀 소멸 → 제외
- 시가 대비 현재가 위치: 시가 아래로 밀린 종목은 제외 (갭상승 실패)
- 고점 대비 현재 위치 반드시 계산
    ...
    def _build_user_prompt(
        ...
        if is_market_open:
            lines.append(
                ...
                "반드시 5분봉 데이터를 분석하세요: "
                "양봉 연속+거래량 증가=진입 가능, 음봉 연속 or 거래량 감소=진입 금지. "
                "시가 대비 현재가가 아래면 갭상승 실패이므로 제외하세요. "
